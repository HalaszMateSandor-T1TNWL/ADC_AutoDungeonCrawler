...
{
	[Signal] public delegate void AddRoomToListEventHandler(Rect2 room);
	[Signal] public delegate void ClearRoomsInListEventHandler();
	[Signal] public delegate void DebugRoomEventHandler(Rect2 room);
	...
	public void EmitList()
	{
		...
		{
			EmitSignal(nameof(AddRoomToList), rooms[i]);
		}
	}
	...
	public void OnRegenerate()
	{
		...
		EmitSignal(nameof(ClearRoomsInList));
	...
	public void GenerateMap()
	{
		...
		for (int i = 0; i < _maxRooms; i++)
		{
			...
			if (PlaceRoom(room))
			{
				...
				EmitSignal(nameof(DebugRoom), room);
				rooms.Add(room);
				GD.Print("[" + i + "]" + " Top left: " + room.Position + " Bottom Right: " + room.End);
			}
		}
	}
